...
function SiteWorkersPanel({site,allWorkers,weekLabel,scopes,setDetailId,setPage}){
  const [scopeAlloc,setScopeAlloc]=useState({}); // {workerId: scopeId}

  // Only show workers who have GPS-confirmed sign-ins on this site
  const confirmedWorkers=useMemo(()=>{
    return allWorkers
      .map(w=>{
        const logs=(w.attendanceLogs||[]).filter(l=>
          (l.siteId===site.id||l.siteName===site.name)&&l.signIn&&l.signOut
        );
        if(logs.length===0)return null;
        const totalHrs=logs.reduce((a,l)=>{
          return a+Math.round(((new Date(l.signOut)-new Date(l.signIn))/3600000)*100)/100;
        },0);
        return{...w,confirmedLogs:logs,confirmedHours:totalHrs,labourCost:totalHrs*(w.agreedRate||0)};
      })
      .filter(Boolean);
  },[allWorkers,site.id,site.name,weekLabel]);

  // Forecast workers (in schedule but not yet GPS confirmed)
  ...
  },[allWorkers,confirmedWorkers,site.name]);
  ...
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
      {[
        ["GPS Confirmed",confirmedWorkers.length+" workers","#34d399"],
        ["Forecast Only",forecastWorkers.length+" workers","#fbbf24"],
        ["Confirmed Hours",confirmedWorkers.reduce((a,w)=>a+w.confirmedHours,0).toFixed(1)+"h","#60a5fa"],
    ...
    {confirmedWorkers.length>0&&<div style={{marginBottom:14}}>
      <div style={{fontSize:10,color:"#34d399",fontWeight:700,textTransform:"uppercase",marginBottom:8,display:"flex",alignItems:"center",gap:6}}>
        <span style={{width:8,height:8,borderRadius:"50%",background:"#34d399",display:"inline-block"}}/>
        GPS Confirmed on Site ({confirmedWorkers.length})
      </div>
      <div style={{border:"1px solid #1e2535",borderRadius:10,overflow:"hidden"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          <thead><tr>
            ...
            <th style={{...DS.th,textAlign:"left",minWidth:180}}>Allocate to Scope</th>
          </tr></thead>
          <tbody>
              ...
              <tr key={w.id} style={{background:i%2===0?"#111827":"#0f1421",cursor:"pointer"}}
                ...
                <td style={DS.td}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <div><div style={{fontWeight:600,color:"#f1f5f9",fontSize:12}}>{w.name}</div><div style={{fontSize:10,color:"#64748b"}}>{w.position}</div></div>
                  </div>
                </td>
                ...
                <td style={{...DS.td,textAlign:"right",color:"#f87171",fontWeight:700}}>{w.labourCost>0?fmtM(w.labourCost):"—"}</td>
                <td style={{...DS.td}} onClick={e=>e.stopPropagation()}>
                  {scopes.length>0
                    ?<select value={scopeAlloc[w.id]||""} onChange={e=>setScopeAlloc(a=>({...a,[w.id]:e.target.value}))}
                        style={{width:"100%",background:"#0f1421",border:"1px solid #2d3555",borderRadius:6,padding:"4px 7px",color:scopeAlloc[w.id]?"#e2e8f0":"#64748b",fontSize:11,outline:"none",cursor:"pointer"}}>
                        <option value="">— Unassigned —</option>
                        {scopes.map(sc=><option key={sc.id} value={sc.id}>{sc.description||"Scope item"}</option>)}
                      </select>
                    :<span style={{color:"#374151",fontSize:11}}>No scopes set</span>}
                </td>
              </tr>
            ))}
          </tbody>
