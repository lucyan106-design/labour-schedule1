...
// ─── Financial Dashboard ──────────────────────────────────────────────────────
function FinancialDashboard({workers,clients,allSites,activeDays,siteHours,scopeData,invoices,layoutPrefs,updateLayout,resetLayout}){
  ...
  const DEFAULT_SECTIONS=[
    ...
    {id:"clients",label:"👔 Client P&L"},
  ];
  const [sections,setSections]=useState(()=>{
    if(!layoutPrefs?.financeSections) return DEFAULT_SECTIONS;
    const ordered=[];
    layoutPrefs.financeSections.forEach(id=>{const s=DEFAULT_SECTIONS.find(x=>x.id===id);if(s)ordered.push(s);});
    DEFAULT_SECTIONS.forEach(s=>{if(!ordered.find(x=>x.id===s.id))ordered.push(s);});
    return ordered;
  });

  // Sync sections if layoutPrefs changes from outside (e.g. reset)
  useEffect(()=>{
    if(!layoutPrefs?.financeSections){setSections(DEFAULT_SECTIONS);return;}
    const ordered=[];
    layoutPrefs.financeSections.forEach(id=>{const s=DEFAULT_SECTIONS.find(x=>x.id===id);if(s)ordered.push(s);});
    DEFAULT_SECTIONS.forEach(s=>{if(!ordered.find(x=>x.id===s.id))ordered.push(s);});
    setSections(ordered);
  },[layoutPrefs?.financeSections]);

  const {h,ghost}=useDragSort(sections,(reordered)=>{setSections(reordered);if(updateLayout)updateLayout("financeSections",reordered);});

  const renderSection=(sec)=>{
    if(sec.id==="kpi") return (
      <div key="kpi" {...(editMode?h("kpi"):{})} style={{marginBottom:20,...(editMode?ghost("kpi"):{opacity:1})}}>
        {editMode&&<div style={{fontSize:10,color:"#64748b",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6,display:"flex",alignItems:"center",gap:5}}><span style={{fontSize:13}}>⠿</span>{sec.label}</div>}
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>
          {[["Total Income",grand.income,C.income],["Total Budget",grand.budget,C.budget],["Labour Cost",grand.labour,C.labour],["Invoiced",grand.invoiced,C.inv],["Collected",grand.paid,C.paid]].map(([l,v,c])=>
            <div key={l} style={{background:"#1a1f2e",border:`1px solid ${c}44`,borderRadius:10,padding:"11px 14px"}}>
              ...
              <div style={{fontSize:18,fontWeight:800,color:c}}>£{(v||0).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g,",")} </div>
    ...
    if(sec.id==="clients") return (
      <div key="clients" {...(editMode?h("clients"):{})} style={{...(editMode?ghost("clients"):{opacity:1})}}>
        {editMode&&<div style={{fontSize:10,color:"#64748b",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8,display:"flex",alignItems:"center",gap:5}}><span style={{fontSize:13}}>⠿</span>{sec.label}</div>}
        {!editMode&&<div style={{fontWeight:700,color:"#94a3b8",fontSize:12,marginBottom:10,textTransform:"uppercase",letterSpacing:"0.06em"}}>Client P&L</div>}
        {clientRollup.map(cr=>{
              ...
              {cr.sites.map(r=>{
                ...
                return <div key={r.site.id} style={{display:"flex",alignItems:"center",gap:8}}>
                  ...
                  <div style={{flex:1,display:"flex",flexDirection:"column",gap:2}}>
                    <div style={{display:"flex",alignItems:"center",gap:5}}>{bar(r.income,siteMax,C.income,6)}<span style={{fontSize:9,color:C.income,minWidth:55,textAlign:"right"}}>£{r.income.toFixed(0)}</span></div>
                    <div style={{display:"flex",alignItems:"center",gap:5}}>{bar(r.labour,siteMax,sOver?"#f87171":C.labour,6)}<span style={{fontSize:9,color:sOver?"#f87171":C.labour,minWidth:55,textAlign:"right"}}>£{r.labour.toFixed(0)}</span></div>
                  </div>
  ...
  return <div style={{padding:"14px 18px"}}>
    <div style={{display:"flex",justifyContent:"flex-end",marginBottom:8}}>
      <EditBar editMode={editMode} onToggle={()=>setEditMode(e=>!e)} onReset={()=>{if(resetLayout)resetLayout("financeSections");}} label="Sections"/>
    </div>
...
// ─── Drag-Sort Utilities (no library — pure HTML5 drag API) ──────────────────
function useDragSort(items, onReorder) {
  ...
  const h = (id) => ({
    ...
    onDrop:      (e) => {
      ...
      const arr = [...items];
      const fi  = arr.findIndex(x => (x.id ?? x) === dragId);
      const ti  = arr.findIndex(x => (x.id ?? x) === id);
      if(fi >= 0 && ti >= 0){ arr.splice(ti, 0, arr.splice(fi, 1)[0]); onReorder(arr); }
  ...
  const ghost = (id) => ({
    opacity:     dragId === id ? 0.25 : 1,
    outline:     overId === id && dragId !== id ? "2px dashed #3b82f6" : "2px solid transparent",
    outlineOffset: 4,
    transition:  "opacity 0.12s",
    cursor:      dragId ? "grabbing" : "grab",
  });

  return { h, ghost, isDragging: dragId !== null };
}

// Toolbar that appears above any sortable section
function EditBar({ editMode, onToggle, onReset, label="Layout" }){
  return (
    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:editMode?10:0,minHeight:editMode?28:0}}>
      {editMode && <>
        <span style={{fontSize:11,color:"#64748b",flex:1,fontStyle:"italic"}}>⠿ Drag items to reorder · saves automatically to all devices</span>
        <button onClick={onReset} style={{padding:"3px 9px",background:"none",border:"1px solid #2d3555",borderRadius:5,color:"#64748b",cursor:"pointer",fontSize:11}}>↺ Reset</button>
      </>}
      <button onClick={onToggle}
        style={{padding:"3px 10px",background:editMode?"#1e3a5f":"transparent",border:`1px solid ${editMode?"#3b82f6":"#2d3555"}`,borderRadius:5,color:editMode?"#60a5fa":"#374151",cursor:"pointer",fontSize:11,fontWeight:editMode?700:400,whiteSpace:"nowrap"}}>
...
// Admin-only nav (visible only when adminMode is unlocked)
const ADMIN_NAV=[
  {id:"pending", icon:"🆕", label:"Pending Workers"},
];
...
// ── Pending Workers Approval Panel ───────────────────────────────────────────
function DPendingWorkers({onApprove}){
  ...
  const approve=async(row)=>{
    ...
    try{
      const newWorker=mkW({...row.data,id:row.id});
      await sbUpsert("workers",[{id:row.id,data:newWorker}]);
      await fetch(`${SB_URL}/rest/v1/pending_workers?id=eq.${row.id}`,{method:"PATCH",headers:{...SB_H,"Prefer":"return=representation"},body:JSON.stringify({status:"approved"})});
      setPending(p=>p.map(x=>x.id===row.id?{...x,status:"approved"}:x));
      if(onApprove) onApprove(newWorker);
      alert(`✓ ${row.data?.name} approved and added to workers.`);
  ...
  const reject=async(row)=>{
    ...
    try{
      await fetch(`${SB_URL}/rest/v1/pending_workers?id=eq.${row.id}`,{method:"PATCH",headers:{...SB_H,"Prefer":"return=representation"},body:JSON.stringify({status:"rejected",data:{...row.data,rejectionNote:note,rejectedAt:new Date().toISOString()}})});
      setPending(p=>p.map(x=>x.id===row.id?{...x,status:"rejected"}:x));
    }catch(e){alert("Error: "+e.message);}
    setActioning(a=>({...a,[row.id]:null}));
  };

  const shown=pending.filter(p=>tab==="pending"?p.status==="pending":p.status!=="pending");

  if(loading) return <div style={DS.body}><div style={{color:"#64748b",textAlign:"center",padding:40}}>Loading registrations…</div></div>;

  return <div>
    <DPageHdr title="🆕 Pending Workers" sub="Review and approve new worker registrations"
      actions={<button onClick={load} style={{padding:"6px 12px",background:"#1e2535",border:"1px solid #2d3555",borderRadius:6,color:"#94a3b8",cursor:"pointer",fontSize:12}}>↺ Refresh</button>}/>
    <div style={DS.body}>
      {/* Tab toggle */}
      <div style={{display:"flex",gap:3,background:"#0d1117",borderRadius:8,padding:3,marginBottom:18,width:"fit-content"}}>
        {[["pending","⏳ Awaiting Review"],["processed","✓ Processed"]].map(([v,l])=>(
          <button key={v} onClick={()=>setTab(v)} style={{padding:"6px 16px",background:tab===v?"#1e3a5f":"transparent",border:tab===v?"1px solid #3b82f6":"1px solid transparent",borderRadius:6,color:tab===v?"#60a5fa":"#64748b",cursor:"pointer",fontSize:12,fontWeight:tab===v?700:400}}>
            {l}{v==="pending"&&pending.filter(p=>p.status==="pending").length>0?<span style={{marginLeft:6,background:"#fbbf24",color:"#000",borderRadius:9,fontSize:10,padding:"1px 6px",fontWeight:700}}>{pending.filter(p=>p.status==="pending").length}</span>:null}
          </button>
        ))}
      </div>

      {shown.length===0&&<div style={{textAlign:"center",padding:60,color:"#374151",fontSize:14,border:"1px dashed #1e2535",borderRadius:12}}>
        {tab==="pending"?"✓ No pending registrations — you're all caught up.":"No processed registrations yet."}
      </div>}
      ...
      {shown.map(row=>{
        ...
        const statusColor={pending:"#fbbf24",approved:"#34d399",rejected:"#f87171"}[status]||"#94a3b8";
        const heldCerts=Object.entries(d.certs||{}).filter(([,v])=>v?.held);

        return <div key={row.id} style={{background:"#111827",border:`1px solid ${statusColor}33`,borderRadius:12,marginBottom:12,overflow:"hidden"}}>
          {/* Summary row */}
          <div style={{display:"flex",alignItems:"center",gap:12,padding:"14px 18px",cursor:"pointer"}} onClick={()=>setExpanded(isOpen?null:row.id)}>
            <div style={{width:40,height:40,borderRadius:10,background:statusColor+"22",border:`1px solid ${statusColor}44`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>👤</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontWeight:700,color:"#f1f5f9",fontSize:14}}>{d.name||"Unknown"}</div>
              <div style={{fontSize:11,color:"#64748b",marginTop:2}}>{d.position||"—"} · {d.company||"—"} · Registered: {row.created_at?new Date(row.created_at).toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"}):"—"}</div>
            </div>
            <span style={{fontSize:11,fontWeight:700,color:statusColor,background:statusColor+"18",padding:"3px 10px",borderRadius:6,textTransform:"capitalize",flexShrink:0}}>{status}</span>
            <span style={{color:"#64748b",fontSize:14}}>{isOpen?"▲":"▼"}</span>
          </div>

          {/* Expanded details */}
          {isOpen&&<div style={{borderTop:"1px solid #1e2535",padding:"18px 18px 14px"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginBottom:16}}>
              {/* Personal */}
              <div style={{background:"#0f1421",borderRadius:9,padding:13}}>
                <div style={{fontSize:10,color:"#3b82f6",fontWeight:700,textTransform:"uppercase",marginBottom:9}}>Personal Details</div>
                {[["Email",d.email],["Phone",d.contact],["DOB",d.dob],["NI Number",d.nino?d.nino.replace(/.(?=.{4})/g,"*"):"—"],["Address",d.address],["Next of Kin",d.nextOfKin],["NOK Phone",d.nextOfKinPhone]].filter(([,v])=>v).map(([l,v])=>(
                  <div key={l} style={{display:"flex",gap:6,marginBottom:5,fontSize:12}}><span style={{color:"#64748b",width:90,flexShrink:0}}>{l}</span><span style={{color:"#e2e8f0",wordBreak:"break-word"}}>{v}</span></div>
                ))}
              </div>
              {/* Bank */}
              <div style={{background:"#0f1421",borderRadius:9,padding:13}}>
                <div style={{fontSize:10,color:"#10b981",fontWeight:700,textTransform:"uppercase",marginBottom:9}}>Bank & Documents</div>
                {[["Bank",d.bankName],["Account",d.bankAccount?d.bankAccount.replace(/.(?=.{4})/g,"*"):"—"],["Sort Code",d.bankSort],["UTR",d.utr]].filter(([,v])=>v).map(([l,v])=>(
                  <div key={l} style={{display:"flex",gap:6,marginBottom:5,fontSize:12}}><span style={{color:"#64748b",width:90,flexShrink:0}}>{l}</span><span style={{color:"#e2e8f0"}}>{v}</span></div>
                ))}
                {d.termsAccepted&&<div style={{marginTop:10,padding:"7px 10px",background:"#0d2218",borderRadius:6,border:"1px solid #10b981"}}>
                  <div style={{fontSize:11,color:"#34d399",fontWeight:700}}>✓ T&Cs Signed</div>
                  {d.termsSignedAt&&<div style={{fontSize:10,color:"#64748b",marginTop:2}}>{new Date(d.termsSignedAt).toLocaleString("en-GB")}</div>}
                </div>}
              </div>
              {/* Certs */}
              <div style={{background:"#0f1421",borderRadius:9,padding:13}}>
                <div style={{fontSize:10,color:"#fbbf24",fontWeight:700,textTransform:"uppercase",marginBottom:9}}>Declared Certs ({heldCerts.length})</div>
                {heldCerts.length===0&&<div style={{color:"#374151",fontSize:11}}>None declared</div>}
                {heldCerts.map(([key,v])=>{
                  const cert=CERTS.find(c=>c.key===key);
                  return <div key={key} style={{marginBottom:6,fontSize:11}}>
                    <div style={{color:"#e2e8f0",fontWeight:600}}>{cert?.label||key}</div>
                    {v.expiry&&<div style={{color:"#64748b"}}>Expiry: {v.expiry}</div>}
                    {v.regNo&&<div style={{color:"#64748b"}}>Reg: {v.regNo}</div>}
                    {v.fileUrl&&<a href={v.fileUrl} target="_blank" rel="noreferrer" style={{color:"#60a5fa",fontSize:10}}>📎 View Photo</a>}
                  </div>;
                })}
              </div>
            </div>

            {/* Rejection note + action buttons */}
            {status==="pending"&&<div style={{borderTop:"1px solid #1e2535",paddingTop:14,display:"flex",gap:10,alignItems:"flex-start"}}>
              <input value={rejectNote[row.id]||""} onChange={e=>setRejectNote(n=>({...n,[row.id]:e.target.value}))} placeholder="Rejection reason (optional)…"
                style={{flex:1,background:"#0f1421",border:"1px solid #2d3555",borderRadius:7,padding:"8px 11px",color:"#e2e8f0",fontSize:12,outline:"none"}}/>
              <button onClick={()=>reject(row)} disabled={!!actioning[row.id]} style={{padding:"9px 18px",background:"#2d1515",border:"1px solid #f87171",borderRadius:7,color:"#f87171",cursor:"pointer",fontSize:12,fontWeight:700,flexShrink:0,opacity:actioning[row.id]?0.6:1}}>
                {actioning[row.id]==="rejecting"?"Rejecting…":"✕ Reject"}
              </button>
              <button onClick={()=>approve(row)} disabled={!!actioning[row.id]} style={{padding:"9px 20px",background:"linear-gradient(135deg,#059669,#10b981)",border:"none",borderRadius:7,color:"#fff",cursor:"pointer",fontSize:12,fontWeight:700,flexShrink:0,opacity:actioning[row.id]?0.6:1}}>
                {actioning[row.id]==="approving"?"Approving…":"✓ Approve & Add Worker"}
              </button>
            </div>}
            {status==="rejected"&&d.rejectionNote&&<div style={{marginTop:8,padding:"8px 12px",background:"#2d1515",borderRadius:7,fontSize:12,color:"#f87171"}}>Rejection note: {d.rejectionNote}</div>}
          </div>}
        </div>;
      })}
    </div>
...
// ── Dashboard Sidebar ─────────────────────────────────────────────────────────
function DashSidebar({page,setPage,workers,allSites,clients,invoices,setModal,activeDays,siteHours,weekLabel,adminMode,pendingCount,onAdminToggle}){
  ...
  const badges={certs:expiring,invoices:invoices.filter(i=>i.status==="pending").length,pending:pendingCount||0};
  ...
  return <div style={DS.sidebar}>
    <div style={{padding:"12px 10px",flex:1}}>
      ...
      {/* Admin-only section */}
      {adminMode&&<>
        <div style={{fontSize:9,color:"#7c3aed",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",padding:"10px 10px 4px",opacity:0.8}}>🔓 Admin</div>
        {ADMIN_NAV.map(item=>{
          const active=isActive(item.id);
          const cnt=badges[item.id];
          return <div key={item.id} onClick={()=>setPage(item.id)}
            style={{display:"flex",alignItems:"center",gap:9,padding:"8px 10px",borderRadius:7,marginBottom:2,cursor:"pointer",background:active?"#2e1a5e":"transparent",border:active?"1px solid #7c3aed":"1px solid transparent",transition:"all 0.12s"}}
            onMouseEnter={e=>{if(!active)e.currentTarget.style.background="#1a1335";}}
            onMouseLeave={e=>{if(!active)e.currentTarget.style.background="transparent";}}>
            <span style={{fontSize:15,width:18,textAlign:"center",flexShrink:0}}>{item.icon}</span>
            <span style={{flex:1,fontSize:12,fontWeight:active?700:400,color:active?"#a78bfa":"#7c3aed"}}>{item.label}</span>
            {cnt>0&&<span style={{fontSize:10,fontWeight:700,color:"#fff",background:"#7c3aed",padding:"1px 5px",borderRadius:9,minWidth:18,textAlign:"center"}}>{cnt}</span>}
          </div>;
        })}
      </>}
    </div>
    <div style={{padding:"10px 12px",borderTop:"1px solid #1e2535"}}>
      ...
      {/* Admin lock toggle */}
      <button onClick={onAdminToggle} style={{width:"100%",padding:"7px 10px",background:adminMode?"#1a0d2e":"#0d1117",border:`1px solid ${adminMode?"#7c3aed":"#2d3555"}`,borderRadius:7,color:adminMode?"#a78bfa":"#374151",cursor:"pointer",fontSize:11,fontWeight:700}}>
        {adminMode?"🔓 Admin Mode — Lock":"🔐 Admin Access"}
      </button>
    </div>
  </div>;
}
...
// ── Dashboard Home ────────────────────────────────────────────────────────────
function DHome({workers,allSites,clients,invoices,scopeData,activeDays,siteHours,weeklyRecords,setPage,layoutPrefs,updateLayout,resetLayout}){
  ...
  // Apply saved order, recompute values each render
  const objects = useMemo(()=>{
    if(!layoutPrefs?.homeCards) return DEFAULT_OBJECTS;
    const ordered=[];
    layoutPrefs.homeCards.forEach(id=>{const o=DEFAULT_OBJECTS.find(x=>x.id===id);if(o)ordered.push(o);});
    DEFAULT_OBJECTS.forEach(o=>{if(!ordered.find(x=>x.id===o.id))ordered.push(o);});
    return ordered;
  },[workers,allSites,clients,invoices,layoutPrefs?.homeCards]);

  const {h,ghost}=useDragSort(objects,(reordered)=>{if(updateLayout)updateLayout("homeCards",reordered);});

  return <div>
    <DPageHdr title="🏗 Bright Metalwork" sub="Project Management Overview"
      actions={<EditBar editMode={editMode} onToggle={()=>setEditMode(e=>!e)} onReset={()=>{if(resetLayout)resetLayout("homeCards");}} label="Cards"/>}/>
    <div style={DS.body}>
      ...
      <div style={{fontSize:11,color:"#64748b",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:8}}>All Objects — {editMode?"drag to reorder":"click to open"}</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
        {objects.map(obj=>(
          <div key={obj.id}
            {...(editMode ? h(obj.id) : {})}
            onClick={()=>{ if(!editMode) setPage(obj.id); }}
            style={{...DS.card(obj.color),borderColor:`${obj.color}33`,...(editMode?ghost(obj.id):{})}}
            onMouseEnter={e=>{if(!editMode){e.currentTarget.style.borderColor=obj.color;e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow=`0 8px 30px ${obj.color}22`;}}}
            onMouseLeave={e=>{if(!editMode){e.currentTarget.style.borderColor=`${obj.color}33`;e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";}}}
          >
            <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:`linear-gradient(90deg,${obj.color},${obj.color}44)`}}/>
            {editMode&&<div style={{position:"absolute",top:8,right:10,fontSize:14,color:`${obj.color}88`,userSelect:"none"}}>⠿</div>}
            <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
              ...
              {!editMode&&<span style={{color:`${obj.color}66`,fontSize:16}}>→</span>}
            </div>
          </div>
        ))}
      </div>
...
function DashboardView({workers,allSites,clients,weekLabel,activeDays,siteHours,scopeData,invoices,saveWorker,delWorker,setAllSites,setClients,setModal,weeklyRecords,setWeeklyRecords,showWeekend,filter,setFilter,allSiteNames,updateCell,delWorker,dashPage,setDashPage,dashDetailId,setDashDetailId,adminMode,pendingCount,onAdminToggle,onWorkerApproved,layoutPrefs,updateLayout,resetLayout}){
  ...
  const SP={workers,allSites,clients,invoices,activeDays,siteHours,weekLabel,setModal,scopeData,filter,setFilter,allSiteNames,updateCell,delWorker,showWeekend,weeklyRecords,setWeeklyRecords,layoutPrefs,updateLayout,resetLayout};
  const renderPage=()=>{
    switch(dashPage){
      // ── Overview
      ...
      // ── Admin only
      case "pending":       return adminMode?<DPendingWorkers onApprove={onWorkerApproved}/>:<DComingSoon icon="🔐" title="Admin Access Required" sub="Click 🔐 Admin Access in the sidebar to unlock this section."/>;
  ...
  return (
    <div style={{display:"flex",minHeight:"calc(100vh - 60px)",background:"#080d14"}}>
      <DashSidebar page={dashPage} setPage={p=>{setDashPage(p);setDashDetailId(null);}} workers={workers} allSites={allSites} clients={clients} invoices={invoices} setModal={setModal} activeDays={activeDays} siteHours={siteHours} weekLabel={weekLabel} adminMode={adminMode} pendingCount={pendingCount} onAdminToggle={onAdminToggle}/>
...
export default function App(){
  ...
  const [weeklyRecords,setWeeklyRecords]=useState([]); // saved weekly snapshots
  ...
  const [adminPin,setAdminPin]=useState("1234"); // loaded from DB, changeable by director
  const [pendingCount,setPendingCount]=useState(0);
  // ── Layout preferences (saved to Supabase)
  const [layoutPrefs,setLayoutPrefs]=useState({homeCards:null,statsStrip:null,financeSections:null});
  ...
  useEffect(()=>{
    async function loadAll(){
      try {
        ...
        if(cfg.admin_pin) setAdminPin(cfg.admin_pin);
        if(cfg.layout_prefs) setLayoutPrefs(prev=>({...prev,...cfg.layout_prefs}));
        // Load pending count (don't block on failure)
        try{const pRows=await sbGet("pending_workers","select=id&status=eq.pending");setPendingCount(pRows.length);}catch(e){}
  ...
  const handleAdminToggle=()=>{
    ...
    const entered=window.prompt("Enter admin PIN to unlock:");
    if(entered===null) return; // cancelled
    if(entered===adminPin){
      setAdminMode(true);
    } else if(entered===""){
      // No PIN set yet — allow director to set one
      const newPin=window.prompt("No PIN entered. Set a new admin PIN (or cancel to abort):");
      if(newPin&&newPin.length>=4){
        setAdminPin(newPin);
        sbUpsert("app_config",[{key:"admin_pin",value:newPin}]);
        setAdminMode(true);
        alert("✓ Admin PIN set. You can change it any time from Admin Access.");
      }
    } else {
  ...
  // Called when a pending worker is approved — refresh workers list and pending count
  const handleWorkerApproved=(newWorker)=>{
    setWorkers(ws=>{const exists=ws.find(x=>x.id===newWorker.id);return exists?ws:([...ws,newWorker]);});
    setPendingCount(c=>Math.max(0,c-1));
  };

  // Update one key of layoutPrefs — triggers auto-save to Supabase via the save useEffect
  const updateLayout=(key,orderedItems)=>{
    const ids=orderedItems.map(x=>x.id??x);
    setLayoutPrefs(prev=>({...prev,[key]:ids}));
  };
  const resetLayout=(key)=>{
    setLayoutPrefs(prev=>({...prev,[key]:null}));
  };
    ...
    saveTimer.current=setTimeout(async()=>{
      try {
        await sbUpsert("app_config",[
          ...
          {key:"layout_prefs",value:layoutPrefs},
        ]);
        setSyncStatus("saved");
      } catch(e){ setSyncStatus("error"); }
    },800);
  },[weekLabel,showWeekend,allSites,clients,siteHours,scopeData,invoices,layoutPrefs,loading]);
  ...
  return <div style={{minHeight:"100vh",background:"#0d1117",fontFamily:"system-ui,'Segoe UI',sans-serif",color:"#e2e8f0",fontSize:13,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{background:"linear-gradient(135deg,#0f172a,#1a1f2e)",borderBottom:"1px solid #1e2535",padding:"9px 16px",position:"sticky",top:0,zIndex:200,flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
        ...
        {/* Quick stats strip — drag to reorder */}
        {(()=>{
          const DEFAULT_STATS=[
            {key:"Workers", l:"Workers", v:stats.total,  c:"#60a5fa",  pg:"workers"},
            {key:"Holiday", l:"Holiday", v:stats.onHol,  c:"#fbbf24",  pg:"workers"},
            {key:"Off",     l:"Off",     v:stats.off,     c:"#94a3b8",  pg:"workers"},
            {key:"Cert ⚠",  l:"Cert ⚠",  v:stats.alerts, c:stats.alerts>0?"#f87171":"#34d399",pg:"certs"},
            {key:"Gross",   l:"Gross",   v:stats.g>0?`£${stats.g.toFixed(0)}`:"—",c:"#34d399",pg:"payslips"},
            {key:"Net",     l:"Net",     v:stats.g>0?`£${stats.n.toFixed(0)}`:"—",c:"#a78bfa",pg:"payslips"},
          ];
          const sorted=layoutPrefs.statsStrip
            ?[...DEFAULT_STATS].sort((a,b)=>{const ai=layoutPrefs.statsStrip.indexOf(a.key),bi=layoutPrefs.statsStrip.indexOf(b.key);return(ai<0?999:ai)-(bi<0?999:bi);})
            :DEFAULT_STATS;
          const [statEdit,setStatEdit]=useState(false);
          const {h:sh,ghost:sg}=useDragSort(sorted,(r)=>updateLayout("statsStrip",r));
          return <div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap"}}>
            {sorted.map(s=>(
              <div key={s.key}
                {...(statEdit?sh(s.key):{})}
                style={{background:"#111827",border:statEdit?"1px solid #2d3555":"1px solid #1e2535",borderRadius:7,padding:"3px 10px",cursor:statEdit?"grab":"pointer",...(statEdit?sg(s.key):{})}}
                onClick={()=>{if(!statEdit)setDashPage(s.pg);}}>
            ...
            <button onClick={()=>setStatEdit(e=>!e)}
              title={statEdit?"Done rearranging":"Drag stats to reorder"}
              style={{padding:"3px 7px",background:statEdit?"#1e3a5f":"transparent",border:`1px solid ${statEdit?"#3b82f6":"#1e2535"}`,borderRadius:5,color:statEdit?"#60a5fa":"#2d3555",cursor:"pointer",fontSize:11,fontWeight:statEdit?700:400,lineHeight:1}}>
              {statEdit?"✓":"⊞"}
            </button>
          </div>;
        })()}
    ...
    <div style={{display:"flex",flex:1,overflow:"hidden"}}>
      <div style={{flex:1,overflowY:"auto",height:"calc(100vh - 55px)"}}>
        <DashboardView
          ...
          adminMode={adminMode} pendingCount={pendingCount}
          onAdminToggle={handleAdminToggle}
          onWorkerApproved={handleWorkerApproved}
          layoutPrefs={layoutPrefs} updateLayout={updateLayout} resetLayout={resetLayout}/>
      </div>
    </div>
